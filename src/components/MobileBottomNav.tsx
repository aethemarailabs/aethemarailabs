      ...
      {navItems.map((item) => {
        ...
        return (
          <Link 
            key={item.name} 
            href={item.href} 
            className={`flex flex-col items-center justify-center p-2 transition-all duration-300 ${isActive ? 'text-primary drop-shadow-[0_0_8px_rgba(212,175,55,0.6)] scale-110' : 'text-on-surface-variant hover:text-primary/70'}`}
          >
            <span 
